...
class nsf_HiFigan(GanBaseTask):
    ...
    def Dforward(self, Goutput):
        ...
        return {'msd': msd_out, 'mpd': mpd_out}, {'msd': msd_feature, 'mpd': mpd_feature}
        # return (msd_out, mpd_out), (msd_feature, mpd_feature)
        # return (msd_out, msd_feature), (mpd_out, mpd_feature)

    def lsgan_loss(self, x, real_=True, discriminators=True):
        if discriminators:
            if real_:
    ...
    def one_d_loss(self, d_g_sg, d_real, ):
        loss = 0
        main_dg_loss = 0
        main_dr_loss = 0
        for dg, dr in zip(d_g_sg, d_real):
            loss1 = self.lsgan_loss(dg, real_=False, discriminators=True)
            loss2 = self.lsgan_loss(dr, real_=True, discriminators=True)
            main_dg_loss = main_dg_loss + loss1.detach()
            main_dr_loss = main_dr_loss + loss2.detach()
            loss = loss + loss1 + loss2
        return loss, main_dg_loss, main_dr_loss

    def one_g_loss(self, d_g_sg, d_g, ):
        loss = 0
        main_g_loss = 0
        for dg_sg, dg in zip(d_g_sg, d_g):
            loss1 = self.lsgan_loss(dg, real_=False, discriminators=False)
            loss2 = self.lsgan_loss(dg_sg, real_=False, discriminators=False)
            loss = loss + (loss1 - loss2)
            main_g_loss = main_g_loss + loss1.detach()
        return loss, main_g_loss

    def one_step_gan_loss(self, d_g_sg, d_real, d_g, gan_d_scale=1):

        logss = {}
        losss = 0
        for k in d_g_sg.keys():
            loss_d, main_dg_loss, main_dr_loss = self.one_d_loss(d_g_sg[k], d_real[k])
            loss_g, main_g_loss = self.one_g_loss(d_g_sg[k], d_g[k])
            rc = len(d_g_sg[k])
            logss[f'{k}_loss_d'] = loss_d
            logss[f'{k}_loss_g'] = loss_g
            logss[f'{k}_main_dg_loss'] = main_dg_loss / rc
            logss[f'{k}_main_dr_loss'] = main_dr_loss / rc
            logss[f'{k}_main_g_loss'] = main_g_loss / rc
            losss = losss + loss_d * gan_d_scale + loss_g

        return losss, logss

    def one_f_loss(self, df_g_sg, df_real, df_g):
        logss = {}
        losss = 0
        for k in df_g_sg.keys():
            f_loss = self.feature_loss(df_real[k], df_g[k])
            f_sg_loss = self.feature_loss(df_real[k], df_g_sg[k])
            fx_loss = f_loss - f_sg_loss

            logss[f'{k}_feature_loss'] = f_loss
            losss = losss + fx_loss
    ...
    def feature_loss(self, fmap_r, fmap_g):
        ...
        for dr, dg in zip(fmap_r, fmap_g):
            for rl, gl in zip(dr, dg):
                ...
                loss += torch.mean(torch.abs(rl[: b].detach() - gl[: b]))
        return loss * 2
    ...
    def _training_step(self, sample, batch_idx):
        ...
        d_g_sg, df_g_sg = self.Dforward(Goutput=audio_fake.detach())
        d_real, df_real = self.Dforward(Goutput=sample['audio'])
        d_g, df_g = self.Dforward(Goutput=audio_fake)

        gan_loss, gan_log = self.one_step_gan_loss(d_g_sg, d_real, d_g, gan_d_scale=self.config['gan_d_scale'])
        feature_loss, feature_log = self.one_f_loss(df_g_sg, df_real, df_g)
        log_dict.update(gan_log)
        log_dict.update(feature_log)
        ...
        Gloss = gan_loss + Auxloss + feature_loss
    ...
    def configure_optimizers(self):
        optG = self.build_optimizer(self, optimizer_args=self.config['generater_optimizer_args'])
